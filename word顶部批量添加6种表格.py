...
class DocxBatchTableTool:
    def __init__(self, root):
        self.root = root
        # 修改窗口标题：第二行 → 第一行
        self.root.title("Docx批量添加表格工具（保留图片+第一行插入）")
        ...
        self.blank_lines_after_table = 0  # 表格后保留的空白行数
    ...
    def _insert_table_at_first_line(self, doc, keyword):
        """
        安全插入表格到第一行（保留图片）
        核心逻辑：先在文档末尾创建表格，再通过段落移动到第一行，避免破坏XML结构
        """
        try:
            # 1. 确保文档至少有1个段落（为第一行预留位置）
            if len(doc.paragraphs) == 0:
                doc.add_paragraph("")  # 空段落占位（保证第一行有插入位置）
                self._log("  ⚠️  文档为空，先插入空段落占位")
    ...
    def _process_single_file(self, file_path):
        ...
        try:
            ...
            # 4. 第一行插入表格（保留图片）（修改：调用新的第一行插入方法）
            create_success = self._insert_table_at_first_line(doc, keyword)
    ...
    def _batch_process(self):
        ...
        # 修改日志提示：第二行 → 第一行
        self._log("🚀 开始批量处理docx文件（保留图片+第一行插入表格）...")
