...
3. 表格移至图片上方（无间隔行）
4. 图片左上角标注：试验结果图：
5. 图片下方中间标注：根据文件名_H/_V显示水平/垂直极化
6. “试验结果图：”前面保留一个空行
7. 新增：删除页眉和页尾的所有内容
"""
import os
...
class DocxBatchTool:
    ...
    def _add_image_annotations(self, doc, img_para, bottom_label):
        """为图片添加标注：左上角+下方中间"""
        try:
    ...
    def _process_single_file(self, file_path):
        ...
        try:
            ...
            # 根据文件名判断图片下方标注文本
            if "_H" in file_name:
                img_label_bottom = "水平极化"
            elif "_V" in file_name:
                img_label_bottom = "垂直极化"
            else:
                img_label_bottom = "水平极化"  # 默认值（可根据需求修改）
                self._log(f"  ⚠️  文件名不含_H/_V，使用默认标注：{img_label_bottom}")
            self._log(f"  📌 图片下方标注确定为：{img_label_bottom}")
            ...
            if table and img_para:
                ...
                # 添加图片标注（传入动态的下方标注文本）
                self._add_image_annotations(doc, img_para, img_label_bottom)
            elif not table:
