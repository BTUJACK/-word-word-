...
1. 重命名：将M1_/M2_/M3_/M4_/M5_/Ambient_移动到P1_后（保留原文件）
2. 内容处理：删除指定文本、调整表格、扩展表格、图片标注等
"""
'''
...
class DocxBatchProcessor:
    def __init__(self, root):
        self.root = root
        self.root.title("Docx批量处理工具（重命名+内容修改）")
        ...
        # 界面变量
        self.source_folder = tk.StringVar()    # 源文件目录
        self.output_folder = tk.StringVar()    # 重命名输出目录
    ...
    def _create_gui(self):
        ...
        # 输出文件夹行
        tk.Label(folder_frame, text="输出文件夹：", font=("SimHei", 10), width=10).grid(
            row=1, column=0, sticky=tk.W, padx=(0, 5), pady=(5, 0)
        )
        output_entry = tk.Entry(folder_frame, textvariable=self.output_folder, font=("SimHei", 10))
        output_entry.grid(row=1, column=1, sticky=tk.EW, padx=(0, 5), pady=(5, 0))
        tk.Button(
            folder_frame, text="选择输出文件夹", command=self._select_output_folder,
            font=("SimHei", 10), bg="#E0E0E0", width=12
        ).grid(row=1, column=2, padx=(0, 15), pady=(5, 0))
        ...
        tk.Button(
            btn_frame, text="开始完整处理（先重命名→再修改内容）", 
    ...
    def _rename_and_copy_file(self, old_path, output_folder):
        """复制文件到输出文件夹并修改名称"""
        try:
    ...
    def _batch_rename(self):
        ...
        if not output_folder or not os.path.isdir(output_folder):
            raise Exception("输出文件夹无效，请选择有效的输出文件夹")
        ...
        self._log(f"\n📁 开始批量重命名（共{len(all_files)}个文件）")
        for filename in all_files:
            old_path = os.path.join(source_folder, filename)
            success, msg = self._rename_and_copy_file(old_path, output_folder)
            self._log(f"  {msg}")
        ...
        self._log(f"✅ {result}")
        return output_folder, success_count > 0
    ...
    def _modify_docx_content(self, file_path):
        ...
        self._log(f"  📁 已备份文件：{os.path.basename(backup_path)}")
        ...
        try:
            ...
            # 保存修改后的文档
            doc.save(file_path)
            self._log(f"  ✅ 内容处理完成：{os.path.basename(file_path)}")
            return True
        except Exception as e:
    ...
    def _batch_process_content(self, folder):
        ...
        if not docx_files:
            raise Exception("输出文件夹中未找到任何docx文件")
        
        self._log(f"\n📊 开始内容处理（共{len(docx_files)}个docx文件）")
    ...
    def _batch_process_all(self):
        ...
        try:
            # 清空日志
            self._clear_log()
            self._log("🚀 开始Docx批量处理（重命名→内容修改）")
            
            # 第一步：批量重命名
            output_folder, rename_success = self._batch_rename()
            ...
            # 第二步：批量处理内容
            content_success, content_fail = self._batch_process_content(output_folder)
